...
function RecipeCardComponent({ recipe }: RecipeCardProps) {
  ...
  useEffect(() => {
    ...
    async function fetchImage() {
      // Don't fetch if it's not a placeholder
      if (recipe.imageUrl && !recipe.imageUrl.includes('placehold.co')) {
  ...
  return (
    <Card className="bg-background/80 dark:bg-card/60 backdrop-blur-sm transition-all hover:shadow-lg hover:border-primary/40 overflow-hidden flex flex-col h-full">
