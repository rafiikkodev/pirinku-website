 ...
 * recipe suggestions generated by AI. Image generation is handled separately
...
const RecipeSchema = z.object({
    ...
    imageUrl: z.string().optional().default('https://placehold.co/600x400.png').describe("A data URI of a generated image for the recipe. Defaults to a placeholder."),
});
...
const recipeSuggestionFlow = ai.defineFlow(
  ...
  async input => {
    // Generate recipe text suggestions.
    const {output} = await recipeSuggestionPrompt(input);
    if (!output?.suggestions) {
      return { suggestions: [] };
    }
    
    // Return suggestions with placeholder images.
    // The client can then request image generation for each recipe individually.
    return { 
        suggestions: output.suggestions.map(s => ({...s, imageUrl: 'https://placehold.co/600x400.png'}))
    };
  }
);
